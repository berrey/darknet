...
void find_replace(const char* str, char* orig, char* rep, char* output)
{
    ...
    *p = '\0';
    ...
    printf("replace:%s、%s、%s", buffer, rep, p + strlen(orig));
    free(buffer);
}
...
{
    //input_path=orig+图片名
    //（str,orig,rep,output）判断orig是否在输入路径中，如果不在,把orig赋给输出路径；如果在，则output=rep+图片名
    // find_replace(input_path, "/images/train2014/", "/labels/train2014/", output_path);    // COCO

    find_replace(input_path, "data/obj/", "data/outLable/", output_path); 
    // printf("input_path:%s,output_path:%s",input_path,output_path);
    ...
    // output_path = '/home/dh/project/darknet/data/outLable/33011050001320200340-17.txt';
